...
func <- "RaceID"
# function ---------------------------------------------------------------------
dis.method <- c("pearson","spearman","logpearson","euclidean","kendall")
cls.method <- c("kmedoids","kmeans","hclust")

runRaceID <- function(sets, 
                      min.count.total = 1,
                      min.count = 1,
                      min.cell = 1,
                      batch = NULL,
    ...
    for (set in sets) {
        ...
        tryCatch({
            ...
            elapsed_time <- system.time({
                ...
                sc <- compdist(sc, metric = dis.method)
                sc <- clustexp(sc, sat = FALSE, 
                               cln = num_labels,
                               FUNcluster = cls.method)
                # sc <- findoutliers(sc)
                # sc <- comptsne(sc)
                # sc <- compfr(sc)
                # sc <- compumap(sc, metric = umap.met)
