...
sets <- c("GSE171993", "fig11981034","E-MTAB-6701/Placenta","E-MTAB-6701/Decidua",
          "Muris/FACS/Pancreas","Muris/drop/lung","NM/39","NM/58","NG/E1","NG/E2",
          
          "E-MTAB-3321","Klein","GSE189120","GSE171993","Zeisel","PBMC", #previous
...
func <- "TSCAN"
# function ---------------------------------------------------------------------
cls.method <- c("mclust","kmeans")


runTSCAN <- function(sets, 
                     cls.method = "mclust") {
    ...
    for (set in sets) {
        ...
        tryCatch({
            ...
            elapsed_time <- system.time({
                ...
                # procdata <- preprocess(data) # not recomended
                
                # if (cls.method == "mclust") { num <- c(num_labels -1, num_labels, num_labels +1)}
                # if (cls.method == "kmeans") { num <- num_labels }
                
                data <- as.matrix(data)
                
                transposed_data <- t(data)
                constant_columns <- apply(transposed_data, 2, function(x) var(x) == 0)
                filtered_data <- transposed_data[, !constant_columns]
                
                print(paste0(set," Origin dimension: ",dim(data)))
                print(paste0(set," Filtered dimension: ",dim(filtered_data)))
                
                filtered_data <- t(filtered_data)
                
                lpsmclust <- exprmclust(filtered_data,
                                        clustermethod = cls.method,
                                        clusternum = num_labels) 
                
            })
            
            clusters <- lpsmclust$clusterid
            embed <- lpsmclust$pcareduceres
