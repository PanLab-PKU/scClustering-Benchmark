...
# sets <- c(
#           "GSE189120","GSE171993","PBMC", #previous
#           "fig11981034","GSE137537","E-MTAB-6701/Placenta","E-MTAB-6701/Decidua","GSE125970/Colon","GSE125970/Ileum", #other tissue
#           "Muris/drop/lung","Sapiens/Tongue","Sapiens/Prostate", #downsampling
#           "NM/20","NM/29","NM/39","NM/41","NM/58",
#           "NG/A13","NG/E1","NG/E3"
# )
    ...
    for (set in sets) {
        ...
        tryCatch({
            ...
            elapsed_time <- system.time({
                ...
                if (dim(sce)[2] > 5000) {large = TRUE}
                sce <- runPCA(sce)
                
                if (large) {
                        sce <- sc3(sce, ks = num_labels, n_cores = ncores)
                        sce <- sc3_run_svm(sce, ks = num_labels)
                } else {
                    sce <- sc3(sce, ks = num_labels, n_cores = ncores)
                }
                
                clusters <- colData(sce)
                embed <- sce@int_colData$reducedDims
                
                print(paste0(set," origin labels: ", num_labels))
                print(paste0(set," labels: ",length(unique(clusters[,1]))))
                print(paste0(set," clusters: ",unique(clusters[,1])))
